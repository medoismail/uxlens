...
export function registerCompetitorTool(server) {
    server.tool("competitor_analysis", "Compare a website against its top competitors. First runs a UX audit, then identifies and benchmarks competitors. Returns competitive positioning, score gaps, and advantages.", { url: z.string().url().describe("Your website URL to compare against competitors") }, async ({ url }) => {
        // Step 1: Run audit first to get scores
        const audit = await mcpFetch("/audit", { url });
        if (!audit.success || !audit.data) {
            return {
                content: [{ type: "text", text: "❌ Could not audit the URL. Competitor analysis requires a successful audit first." }],
            };
        }
        // Step 2: Run competitor analysis
        const comp = await mcpFetch("/competitor-analysis", {
            url,
        ...
        if (!comp.success || !comp.data) {
            return {
                content: [{ type: "text", text: "❌ Competitor analysis failed. Could not identify or reach competitors." }],
            };
        }
        const d = comp.data;
