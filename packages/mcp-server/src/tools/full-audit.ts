...
interface VisualAnalysisResponse {
  heatmapZones: { x: number; y: number; width: number; height: number; intensity: number; label: string }[];
  visualAnalysis: {
    layoutScore: number;
...
export function registerFullAuditTool(server: McpServer) {
  server.tool(
    "full_audit",
    "Run a complete UXLens analysis: 9-layer UX audit + screenshot + AI visual analysis + attention heatmap. This is the most comprehensive analysis available — use it when you want everything in one call.",
