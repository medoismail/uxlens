...
export function ErrorState({ message, onRetry }: ErrorStateProps) {
  return (
    <div className="flex flex-col items-center justify-center py-32 px-6 text-center animate-fade-in relative z-[1]">
      ...
      <p className="mt-2 text-[13px] text-foreground/40 max-w-sm leading-relaxed">
        {message}
      </p>
      <button
        onClick={onRetry}
        className="mt-8 inline-flex items-center gap-2 rounded-lg border px-5 py-2.5 text-[13px] font-medium text-foreground transition-all duration-150 hover:border-foreground/20 active:scale-[0.98]"
