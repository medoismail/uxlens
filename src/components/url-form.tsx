    ...
    <form onSubmit={handleSubmit} className="w-full">
      <div className="focus-glow relative overflow-hidden rounded-[14px] border transition-all duration-300" style={{ background: "var(--s1)", borderColor: "rgba(0,0,0,0.03)" }}>
        ...
        <div className="flex items-center gap-2.5 px-5 py-3.5 border-b" style={{ borderColor: "rgba(0,0,0,0.03)" }}>
          ...
          <div className="flex-1 relative ml-2">
            ...
            <input
              ...
              disabled={isLoading}
              className="w-full bg-transparent text-[12px] font-mono text-foreground/40 placeholder:text-foreground/40 focus:outline-none disabled:opacity-50"
            />
          </div>
        </div>
        ...
        <div className="flex items-center justify-between px-5 py-3 gap-4" style={{ background: "oklch(0 0 0 / 4%)" }}>
          <p className="text-[10px] font-mono text-foreground/45 tracking-wide hidden sm:block">
            Paste your URL and run the full 10-layer audit
          </p>
          <button
            type="submit"
            disabled={isLoading || !url.trim()}
            className="inline-flex items-center gap-2 rounded-lg px-6 py-2.5 text-[14px] font-bold transition-all duration-200 hover:translate-y-[-1px] active:scale-[0.98] disabled:opacity-40 disabled:pointer-events-none disabled:translate-y-0 disabled:shadow-none shrink-0 sm:ml-auto"
